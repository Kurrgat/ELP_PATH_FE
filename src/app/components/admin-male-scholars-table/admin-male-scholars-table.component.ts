...
@Component({
  selector: 'app-admin-male-scholars-table',
  
  templateUrl: './admin-male-scholars-table.component.html',
  styleUrl: './admin-male-scholars-table.component.scss'
})
export class AdminMaleScholarsTableComponent {
...
//sort functionality
  ...
  getScholarsByYear() {
    ...
    if(this.selectedYear == 'all'){
      ...
      url =this.http.serverUrl+'scholars/scholar-males?ignoreCondition=true'
    }else{
      //http://52.15.152.26:5555/scholars/scholar-females?year=2012&ignoreCondition=false
      url = `${this.http.serverUrl}scholars/scholar-males?year=${this.selectedYear}&ignoreCondition=false`
    }
    console.log(url);
